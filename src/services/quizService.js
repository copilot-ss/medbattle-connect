...
function questionKey(question) {
  ...
  const difficulty =
    typeof question.difficulty === 'string' ? question.difficulty.trim().toLowerCase() : '';
...
function buildOfflineSeedQuestions(difficulty, limit, category, language) {
  ...
  const sameCategoryAndDifficulty = normalizedCategory
    ? languagePool.filter(
        (question) =>
          question?.category === normalizedCategory &&
          question?.difficulty === difficulty
      )
    : languagePool.filter((question) => question?.difficulty === difficulty);

  const sameCategoryAnyDifficulty = normalizedCategory
    ? languagePool.filter((question) => question?.category === normalizedCategory)
    : [];

  const anyCategorySameDifficulty = languagePool.filter(
    (question) => question?.difficulty === difficulty
  );

  return mergeQuestionPools(
    shuffleList(sameCategoryAndDifficulty),
    mergeQuestionPools(
      shuffleList(sameCategoryAnyDifficulty),
      mergeQuestionPools(shuffleList(anyCategorySameDifficulty), shuffleList(languagePool), limit),
      limit
    ),
    limit
  );
...
export async function fetchQuestions(
  ...
  const topUpWithOfflineSeeds = (baseQuestions) => {
    const primary = shuffleList(
      normalizeQuestionList(baseQuestions, normalizedDifficulty)
    );
    if (primary.length >= normalizedLimit) {
      return primary.slice(0, normalizedLimit);
    }

    const seedPool = normalizeQuestionList(
      buildOfflineSeedQuestions(
        normalizedDifficulty,
        Math.max(normalizedLimit * 3, 18),
        normalizedCategory,
        normalizedLanguage
      ),
      normalizedDifficulty
    ...
    return mergeQuestionPools(primary, shuffleList(seedPool), normalizedLimit);
  };

  const resolveCachedQuestions = async () => {
    const cached = await loadCachedQuestions(storageKey);
    const resolved = topUpWithOfflineSeeds(cached);
    if (!resolved.length) {
      return [];
    }
    questionsCache.set(cacheKey, { data: resolved, fetchedAt: Date.now() });
    return cloneQuestions(resolved);
  };

  const resolveOfflineSeeds = () => {
    const offlineQuestions = topUpWithOfflineSeeds([]);
  ...
  try {
    ...
    const resolved = topUpWithOfflineSeeds(rows);
    ...
    questionsCache.set(cacheKey, { data: resolved, fetchedAt: now });

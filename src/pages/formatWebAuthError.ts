type FormatWebAuthErrorOptions = {
  fallback?: string;
...
const FRIENDLY_ERROR_MAPPINGS = [
  ...
  },
  {
    ...
    message: 'Bitte bestaetige zuerst deine E-Mail.',
  },
  {
    patterns: [
  ...
  },
  {
    ...
    message: 'Bitte waehle ein staerkeres Passwort.',
  },
];
...
export function formatWebAuthError(
  error: unknown,
  options: FormatWebAuthErrorOptions = {}
) {
  const fallback = options.fallback ?? DEFAULT_GENERIC_MESSAGE;
  const rawMessage =
    typeof error === 'string'
      ? error

...
## Erledigt
- [x] Energie-Balance reduziert (2026-04-17): Standard-Max-Energie startet jetzt bei `5` statt `10`, Rewarded Ads geben nur noch `+2` statt `+5`, und Legacy-Energiebasen werden beim Laden auf den neuen Standard normiert, damit Altstaende nicht auf dem alten 10er-Startwert bleiben.
- [x] Allgemeinwissen-Kategorien weiter ausgebaut (2026-04-16): sichtbarer Category-Catalog enthaelt jetzt neben `Geografie` und `Politik` auch `Geschichte`, `Brainrot` und `Tiere`; alle drei neuen Bereiche haben direkte DE/EN-Offlineseeds fuer Solo-Quizze und bleiben fuer Multiplayer bewusst noch gesperrt, bis ein eigener Online-Fragenpool existiert.
- [x] Google-Play-Policy-Update April 2026 repo-seitig beruecksichtigt (2026-04-16): Avatar-Galerieauswahl fordert auf Android keine breite Medien-Permission mehr an, Android blockt Kontakte-/Location-Permissions explizit, und die Play-Dokumente nennen die aktuelle Relevanz der neuen Contacts-, Location-, Photo/Video- und Health-Hinweise. Manuell offen bleiben weiterhin `Data safety` und `Health apps declaration` in der Play Console.
- [x] Geografie- und Politik-Quiz auf Branch `feature/scalable-category-catalog` als erster Ausbau spielbar gemacht (2026-04-15): beide Kategorien sind im Home jetzt freigeschaltet, haben eigene DE/EN-Solo-Fragenpools fuer Offline- und Fallback-Betrieb und zeigen in der Kategorieansicht bewusst noch gesperrte Lobbys, bis dafuer ein eigener Online-Fragenpool ausgerollt ist.
- [x] Supabase-Cleanup auf Branch `feature/scalable-category-catalog` vorbereitet (2026-04-15): redundante Client-Fallbacks fuer Freunde, Public Profile und Leaderboard auf aktuelle RPCs reduziert; neue Migration konsolidiert `get_leaderboard` und Avatar-Sync, entfernt Legacy-Tabellen `friends_table`/`match_results` sowie ungenutzte Legacy-Indizes.
- [x] Kategorien-Architektur auf Branch `feature/scalable-category-catalog` vorbereitet (2026-04-14): sichtbare App-Kategorien laufen jetzt ueber einen zentralen Catalog mit `Medizin` als aktivem Sammelbereich sowie vorbereiteten `Geografie`-/`Politik`-Slots; Home und Category-Detail sind darauf umgestellt, Offline-/Kategorieauflosung aggregiert medizinische Unterkategorien, und eine neue Supabase-Migration erweitert `get_questions` fuer Kategoriegruppen.
- [x] Repo-Cleanup: ungenutztes Vite-Web-Scaffold, alte `temp_emu_screen*.png`-Screenshots sowie lokale Temp-Ordner `.tmp_*` und `temp/` entfernt; `.gitignore` blockiert die `temp_emu_*`-Artefakte jetzt ebenfalls (2026-04-14).
- [x] Refactor: `PublicProfileSheet` in Hook, Utils und Styles aufgeteilt (2026-04-14), damit die Komponente selbst deutlich kuerzer und wartbarer ist.
- [x] Cleanup: ungenutzte Hook-Rueckgaben in `usePublicProfileSheet` und `useLobbyInviteMonitor` entfernt (2026-04-14), damit die APIs schlanker bleiben.
